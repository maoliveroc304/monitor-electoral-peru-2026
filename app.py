...
def load_data():
    ...
    # --- B. DATOS CSV LOCALES (CARGA BLINDADA) ---
    ...
    # 6. INTERNET QUINTILES (Lógica Nueva para archivo con 3 columnas: Quintil, Rural, Urbana)
    df_internet = pd.DataFrame()
    path_internet = os.path.join(data_path, "internet_quintiles.csv")
    if os.path.exists(path_internet):
        try:
            # Leer asumiendo que el archivo está bien formado (Quintil, Rural, Urbana)
            df_temp = pd.read_csv(path_internet)
            # Normalizar columnas por si acaso
            df_temp.columns = df_temp.columns.str.strip()
            if 'Quintil' in df_temp.columns and 'Rural' in df_temp.columns:
                # Transformar para graficar (melt)
                df_internet = df_temp.melt(id_vars=['Quintil'], value_vars=['Rural', 'Urbana'], var_name='Área', value_name='Porcentaje')
        except: pass
...
# Helper para gráficos con slider y zoom inicial de 5 años
def plot_zoom_chart(df, x_col, y_col, color, chart_type='line'):
    ...
    fig.update_layout(
        xaxis=dict(
            rangeslider=dict(visible=True), # Barra inferior
            type="linear",
            range=[min_x, max_x + 0.5] # Zoom por defecto
        ),
    ...
    with tabs[5]: # Infraestructura
        ...
        with c2:
            ...
            if not df_internet.empty: 
                # Gráfico agrupado Rural/Urbana con slider no es común, pero posible. Aquí mantenemos barras agrupadas.
    ...
    with tabs[6]: # Ambiente
        ...
        with c2:
            ...
            if not df_co2.empty: 
                ...
                fig_co2.update_yaxes(fixedrange=False) # Permitir zoom en Y
